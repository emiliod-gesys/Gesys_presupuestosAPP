...
import { odooExternalRef } from "@/lib/odoo/import-records"
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  for (const row of items) {
    if (
      ...
      !["account.move", "account.payment"].includes(row.model) ||
    ...
    const marker = odooExternalRef(row.model, row.odooId)
    ...
    const desc = `[Odoo] ${row.label}: ${row.name}${partner ? ` — ${partner}` : ""}`
    const dateStr =
